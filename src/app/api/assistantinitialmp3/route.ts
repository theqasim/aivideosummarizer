...
export async function POST(req: Request) {
  const { mp3Transcript } = await req.json();

  let assistant_id = process.env.MP3FILEASSISTANT_ID || "";
  ...
  try {
    ...
    await openai.beta.threads.messages.create(thread.id, {
      role: "user",
      content: `Hi, here is the mp3 file transcript, understand it and give me a detailed summary: "${mp3Transcript}".`,
    });
    ...
    const highlightsMessage = await openai.beta.threads.messages.create(
      thread.id,
      {
        role: "user",
        content:
          "Generate a bullet point list of the highlights for the mp3 file transcript",
      }
    );
