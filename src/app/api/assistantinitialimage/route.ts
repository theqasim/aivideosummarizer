...
export async function POST(req: Request) {
  ...
  let assistant_id = process.env.IMAGEASSISTANT_ID || "";
  ...
  try {
    ...
    await openai.beta.threads.messages.create(thread.id, {
      role: "user",
      content: `Hi, here is the contents of the image, understand it and give me a detailed summary: "${imageText}".`,
    });

    const run = await openai.beta.threads.runs.create(thread.id, {
      assistant_id: assistant.id,
      instructions: "Do not assume the image is made by the user",
    });
    ...
    const highlightsMessage = await openai.beta.threads.messages.create(
      thread.id,
      {
        role: "user",
        content:
          "Generate a bullet point list of the highlights for the image content",
      }
    );
