...
export async function POST(req: Request) {
  const { powerpointText } = await req.json();

  let assistant_id = process.env.POWERPOINTASSISTANT_ID || "";
  ...
  try {
    ...
    await openai.beta.threads.messages.create(thread.id, {
      role: "user",
      content: `Hi, here is the contents of the powerpoint, understand it and give me a detailed summary: "${powerpointText}".`,
    });

    const run = await openai.beta.threads.runs.create(thread.id, {
      assistant_id: assistant.id,
      instructions: "Do not assume the powerpoint is made by the user",
    });
    ...
    const highlightsMessage = await openai.beta.threads.messages.create(
      thread.id,
      {
        role: "user",
        content:
          "Generate a bullet point list of the highlights for the entire powerpoint",
      }
    );
