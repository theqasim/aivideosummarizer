...
export async function POST(req: Request) {
  const { sstTranscript } = await req.json();

  let assistant_id = process.env.SPEECHTOTEXTASSISTANT_ID || "";
  ...
  try {
    ...
    await openai.beta.threads.messages.create(thread.id, {
      role: "user",
      content: `Hi, here is the speech to text transcript, understand it and give me a detailed summary: "${sstTranscript}".`,
    });
    ...
    const highlightsMessage = await openai.beta.threads.messages.create(
      thread.id,
      {
        role: "user",
        content:
          "Generate a bullet point list of the highlights for the speech to text transcript",
      }
    );
