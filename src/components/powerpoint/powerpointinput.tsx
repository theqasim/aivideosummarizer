...
interface PdfInputProps {
  ...
  setPowerpointFile: React.Dispatch<React.SetStateAction<File | null>>;
  setPdfFileName: React.Dispatch<React.SetStateAction<string>>;
}

export default function PowerpointInput({
  setVideoSummary,
  setVideoTitle,
  setThreadId,
  setHighlights,
  ...
  const handlePowerpointUpload = async (event: any) => {
    setLoadingVisibility("block");
    setCloseVisibility("none");
    setLoadingText("Processing Powerpoint...");
    setLoadingTextColor("text-black");
    setIsLoading(true);
    ...
    if (
      ...
      try {
        ...
        try {
          ...
          console.log("Extracted Document Text:", extractedText);

          let endpoint = "/api/assistantinitialpowerpoint";
          ...
          const summaryResponse = await fetch(endpoint, {
            ...
            body: JSON.stringify({ powerpointText: extractedText }),
          });
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 dark:bg-dot-white/[0.2] bg-dot-black/[0.2]">
      ...
      >
        AI Powerpoint Summariser Chatbot
      </h1>
      <p className="text-lg text-center mb-8 animate-fade-up animate-once animate-duration-[500ms]">
        Discover insights from powerpoints in seconds. Simply upload a
        powerpoint to receive a detailed summary, the highlights, and access to
        a chatbot that&apos;s trained specifically on the powerpoint&apos;s
        content.
      </p>
